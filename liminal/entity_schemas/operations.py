...
class CreateEntitySchema(BaseOperation):
    ...
    def describe_operation(self) -> str:
        return f"{self._validated_schema_properties.name}: Creating new entity schema with fields: {','.join([f.warehouse_name for f in self.fields if f.warehouse_name])}."
...
class CreateEntitySchemaField(BaseOperation):
    ...
    def execute(self, benchling_service: BenchlingService) -> dict[str, Any]:
        assert self.field_props.warehouse_name is not None
        try:
            field = TagSchemaModel.get_one(
                benchling_service, self.wh_schema_name
            ).get_field(self.field_props.warehouse_name)
        except ValueError:
            field = None
        if field is None:
            return self._execute_create(benchling_service)
            ...
            if field.archiveRecord is None:
                raise ValueError(
                    f"Field {self.field_props.warehouse_name} is already active on entity schema {self.wh_schema_name}."
                )
            dropdowns_map = get_benchling_dropdown_id_name_map(benchling_service)
            if self.field_props == convert_tag_schema_field_to_field_properties(
                ...
                return UnarchiveEntitySchemaField(
                    self.wh_schema_name, self.field_props.warehouse_name, self.index
                ).execute(benchling_service)
            else:
                raise ValueError(
                    f"Field {self.field_props.warehouse_name} on entity schema {self.wh_schema_name} is different in code versus Benchling."
                )
    ...
    def _execute_create(self, benchling_service: BenchlingService) -> dict[str, Any]:
        ...
        existing_new_field = next(
            (
                ...
                if f.systemName == self.field_props.warehouse_name
            ),
            None,
        )
        if existing_new_field:
            raise ValueError(
                f"Field {self.field_props.warehouse_name} already exists on entity schema {self.wh_schema_name} and is {'archived' if existing_new_field.archiveRecord is not None else 'active'} in Benchling."
            )
        index_to_insert = (
    ...
    def describe_operation(self) -> str:
        return f"{self.wh_schema_name}: Creating entity schema field '{self.field_props.warehouse_name}' at index {self.index}."

    def describe(self) -> str:
        return f"{self.wh_schema_name}: Entity schema field '{self.field_props.warehouse_name}' is not defined in Benchling but is defined in code."
